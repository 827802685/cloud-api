...
- **对外 URL**：`{GATEWAY_MASTER_URL}/api/admin/...`（Admin Pages 根 URL；外部集成方约定使用同名环境变量。例如创建 Key：`POST .../api/admin/keys`）。由 **Admin Worker**（`packages/admin`，单 Worker 二合一，含 Proxy 逻辑）提供，**Proxy 逻辑不提供 `/admin`**。
