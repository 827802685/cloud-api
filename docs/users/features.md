...
Octafuse Gateway 是可自托管的 **AI 能力网关与运营控制面**：统一接入 Chat、图片生成 / 编辑、语音转写、可扩展智能体工具、私有模型服务和多上游供应商，并在一个 Gateway 入口上管理路由、密钥、预算、计费、日志与审计。
...
| 预算与计费 | 按用户 Key 记录请求、Token、成本与扣费，支持周期预算和用量查询。模型与智能体工具均区分 **供应成本（Metered）**、**目录标准价（Standard）**、**用户计费（Charged）** 三笔账（工具在 catalog 直接配绝对单价，无 Route factor/schedule）；`budget_spent` 只累加 charged。模型路由可配基础倍率与**每日时段（Daily schedule）**倍率（业务时区下的高峰 / 闲时），对齐各家模型按时段定价。 |
...
| 调试台 / 模拟器（Playground / Simulator） | **调试台**：Routes 模式直连单条 `model_routes` 上游（不计费、不写日志、无 failover）；Tools 模式读 `system_config` catalog **直连引擎**（可测非活跃引擎，验证密钥；同样不计费、不写日志）。**模拟器**：浏览器打真实代理服务（鉴权、路由、计费、日志）；Kind 含 LLM / Image / Audio / **Tools**（`POST /v1/tools/*`），Tools 模式隐藏 model/route，响应链到智能体工具 → 工具调用记录（Tools → Invocations）。 |
