...
2.0 按 Request Surface → Route Pool → Upstream Target 解析路由；Pool 内以 priority 分层，同层使用 `cache_affinity`（默认）/ `weighted_random` / `fixed_order` / `weighted_round_robin` 与 weight 排序，并按 Provider 维度熔断。一个 Provider 维护一把上游 API Key。
